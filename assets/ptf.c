/*  
    Only loads ASCII PLY files with the attributes: vertex, normal, color, index
    ...
    while(fgets(line, 256, f) != NULL)
    {
        ...
        if(mode == 1)
        {
            ...
            if(sscanf(line, "%f %f %f %f %f %f %u %u %u", &vx, &vy, &vz, &nx, &ny, &nz, &r, &g, &b) == 9)
            {
                ...
                sprintf(add, "%g,%g,%g,", roundf(nx*127.f), roundf(ny*127.f), roundf(nz*127.f));
